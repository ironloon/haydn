...
- [ ] **VM-01**: Stack-based virtual machine executes all Haydn instructions correctly
- [ ] **VM-02**: Stack operations: push, dup, swap, drop, rotate
- [ ] **VM-03**: Arithmetic operations: add, sub, mul, div, mod
- [ ] **VM-04**: Comparison operations: eq, gt, lt
- [ ] **VM-05**: Control flow: conditional jump, unconditional jump, loop
- [ ] **VM-06**: I/O operations: print number, print character, read input
- [ ] **VM-07**: Memory operations: store to address, load from address
- [ ] **VM-08**: VM handles all specified edge cases gracefully per the language spec

### Tuning System

- [ ] **TUNE-01**: Tuning file format documented and parseable (TOML-based configuration)
- [ ] **TUNE-02**: Tuning files map musical gestures (notes, intervals, pitch ranges) to VM opcodes
- [ ] **TUNE-03**: Default piano tuning ships with the binary (works out of the box with MIDI keyboard)
...
- [ ] **TUNE-06**: User can create custom tuning files for any instrument or style
- [ ] **TUNE-07**: Tuning file validation with clear error messages on misconfiguration

### MIDI Input

- [ ] **MIDI-01**: Real-time MIDI input captures note-on/note-off events from any connected MIDI device
- [ ] **MIDI-02**: MIDI note events map to language operations via the active tuning file
- [ ] **MIDI-03**: MIDI input latency is imperceptible during live performance (<5ms)
...
- [ ] **DOC-07**: Tuning file format documentation with examples for creating custom tunings
